...
| 層 | 選擇 | 說明 |
|---|---|---|
| 語言 | Python 3.11+ | 與 AI/LLM 生態最密切 |
| MCP 框架 | [FastMCP](https://github.com/modelcontextprotocol/python-sdk) | 官方 Python SDK |
| HTTP Client | `httpx` | 支援同步/非同步呼叫外部 API |
| 地理運算 | `shapely`、`geopandas` | polygon、村里界相交運算 |
| 設定檔 | YAML + [Pydantic](https://docs.pydantic.dev/) schema 驗證 | Lake Catalog 可由非工程師維護 |
| 本地 cache | SQLite + 內建 DEM/村里界檔案 | MVP 階段不依賴外部 DB |
| LLM(Tool 5) | Anthropic Claude API(可替換 OpenAI) | 態勢摘要生成 |
| 文件 | OpenAPI(Swagger)+ Markdown | 供其他系統以 HTTP 方式整合 |
| HTTP API | FastAPI(與 FastMCP 共用 Pydantic schema) | 同元件以 REST 形式同時暴露,供 web 前端消費 |
| 前端框架 | Nuxt 3(Vue 3) | Reference Client,示範 web 端消費 MCP 元件 |
| 地圖視覺化 | Leaflet + OpenStreetMap tile | GeoJSON polygon、堰塞湖位置 marker |
| Chat 串流 | Server-Sent Events(SSE) | LLM Agent 工具調用過程即時顯示 |
| 部署 | Docker + `docker-compose.yml`(雙服務) | 機關內網或雲端皆可獨立部署 |
| 版控 | GitHub 公開 repo,MIT 授權 | 符合競賽簡章 §8(三)開源規範 |
...
├── mcp_server/           ← Python:6 Tools 核心 + FastMCP + FastAPI
│   ├── pyproject.toml
│   ├── src/barrier_lake_ops/
│   ├── tests/
│   └── Dockerfile
├── web_demo/             ← Nuxt 3 前端(Reference Client)
│   ├── package.json
│   ├── nuxt.config.ts
│   ├── pages/
│   ├── components/
│   └── Dockerfile
└── docker-compose.yml    ← 同時啟動 MCP/REST 服務與前端
```
